...
tecido_controller = Blueprint('tecido',__name__,url_prefix='/tecidos')
tecido_schema = TecidoSchema()

@tecido_controller.route('',methods=['GET'])
@security_token()
def get():
    pagination = Tecido.paginate_search()
    return utils.response_paged(schema=TecidoSchema, pagination=pagination)

@tecido_controller.route('/all',methods=['GET'])
@security_token()
def get_all():
    tecidos = Tecido.all()
...
@tecido_controller.route('/<int:id>',methods=['GET'])
@security_token()
def get_by_id(id):
...
@tecido_controller.route('',methods=['POST'])
@security_token()
def post():
...
@tecido_controller.route('/<int:id>',methods=['PUT'])
@security_token()
def put(id):
    tecido = Tecido.filter(id=id)
    if not tecido:
