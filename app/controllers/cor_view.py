...
cor_view = Blueprint('cor',__name__,url_prefix='/cor')
cor_schema = CorSchema()

@cor_view.route('',methods=['GET'])
@security_token
def get():
...
@cor_view.route('/all',methods=['GET'])
@security_token
def get_all():
    cores = Cor.all()
...
@cor_view.route('/<int:id>',methods=['GET'])
@security_token
def get_by_id(id):
...
@cor_view.route('/<int:id>',methods=['PUT'])
@security_token
def put(id):
    cor = Cor.filter(id=id)
    if not cor:
